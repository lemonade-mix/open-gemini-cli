...
export const EXTENSIONS_DIRECTORY_NAME = path.join(".kaidex", "extensions");
...
export function loadExtensions(workspaceDir) {
    ...
    for (const extension of allExtensions) {
        if (!seenNames.has(extension.config.name)) {
            logger.info(`Loading extension: ${extension.config.name} (version: ${extension.config.version})`);
