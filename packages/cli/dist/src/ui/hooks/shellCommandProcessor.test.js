...
describe("useShellCommandProcessor", () => {
    ...
    beforeEach(() => {
        ...
        mockKaiDexClient = { addHistory: vi.fn() };
    ...
    const renderProcessorHook = () => renderHook(() => useShellCommandProcessor(addItemToHistoryMock, setPendingHistoryItemMock, onExecMock, onDebugMessageMock, mockConfig, mockKaiDexClient));
    ...
    it("should initiate command execution and set pending state", async () => {
        ...
        expect(addItemToHistoryMock).toHaveBeenCalledWith({ type: "user_shell", text: "ls -l" }, expect.any(Number));
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, "/test/dir", expect.any(Function), expect.any(Object), false);
    ...
    it("should handle successful execution and update history correctly", async () => {
        ...
        expect(addItemToHistoryMock.mock.calls[1][0]).toEqual(expect.objectContaining({
            tools: [
                expect.objectContaining({
        ...
        expect(mockKaiDexClient.addHistory).toHaveBeenCalled();
    });
    it("should handle command failure and display error status", async () => {
        ...
        const execPromise = onExecMock.mock.calls[0][0];
        act(() => {
            resolveExecutionPromise(createMockServiceResult({ exitCode: 127, output: "not found" }));
        });
        await act(async () => await execPromise);
        ...
        expect(finalHistoryItem.tools[0].resultDisplay).toContain("Command exited with code 127");
    ...
    describe("UI Streaming and Throttling", () => {
        ...
        it("should throttle pending UI updates for text streams", async () => {
            ...
            expect(setPendingHistoryItemMock).toHaveBeenLastCalledWith(expect.objectContaining({
    ...
    it("should not wrap the command on Windows", async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith("dir", "/test/dir", expect.any(Function), expect.any(Object), false);
    });
    it("should handle command abort and display cancelled status", async () => {
        ...
        act(() => {
            ...
            resolveExecutionPromise(createMockServiceResult({ aborted: true, output: "Canceled" }));
        });
        await act(async () => await execPromise);
        const finalHistoryItem = addItemToHistoryMock.mock.calls[1][0];
        expect(finalHistoryItem.tools[0].status).toBe(ToolCallStatus.Canceled);
        expect(finalHistoryItem.tools[0].resultDisplay).toContain("Command was cancelled.");
    });
    it("should handle binary output result correctly", async () => {
        ...
        expect(finalHistoryItem.tools[0].resultDisplay).toBe("[Command produced binary output, which is not shown.]");
    });
    ...
    describe("Directory Change Warning", () => {
        it("should show a warning if the working directory changes", async () => {
            ...
            await act(async () => await execPromise);
            const finalHistoryItem = addItemToHistoryMock.mock.calls[1][0];
            expect(finalHistoryItem.tools[0].resultDisplay).toContain("WARNING: shell mode is stateless; the directory change to '/test/dir/new' will not persist.");
            expect(vi.mocked(fs.unlinkSync)).toHaveBeenCalledWith(tmpFile);
        });
