...
import { KaiDexEventType as ServerKaiDexEventType, getErrorMessage, isNodeError, MessageSenderType, logUserPrompt, GitService, UnauthorizedError, UserPromptEvent, DEFAULT_KAIDEX_FLASH_MODEL, logConversationFinishedEvent, ConversationFinishedEvent, ApprovalMode, parseAndFormatApiError, getCodeAssistServer, UserTierId, } from "@google/kaidex-cli-core";
...
function showCitations(settings, config) {
...
/**
 * Manages the KaiDex stream, including user input, command processing,
 * API interaction, and tool call lifecycle.
 */
export const useKaiDexStream = (geminiClient, history, addItem, config, settings, onDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, onEditorClose, onCancelSubmit) => {
    ...
    const handleContentEvent = useCallback((eventValue, currentKaiDexMessageBuffer, userMessageTimestamp) => {
        if (turnCancelledRef.current) {
            // Prevents additional output after a user initiated cancel.
            return "";
        }
        let newKaiDexMessageBuffer = currentKaiDexMessageBuffer + eventValue;
        if (pendingHistoryItemRef.current?.type !== "gemini" &&
            ...
            newKaiDexMessageBuffer = eventValue;
        }
        // Split large messages for better rendering performance. Ideally,
        // we should maximize the amount of output sent to <Static />.
        const splitPoint = findLastSafeSplitPoint(newKaiDexMessageBuffer);
        if (splitPoint === newKaiDexMessageBuffer.length) {
            // Update the existing message with accumulated content
            setPendingHistoryItem((item) => ({
                type: item?.type,
                text: newKaiDexMessageBuffer,
            }));
        }
        else {
            // This indicates that we need to split up this KaiDex Message.
            ...
            const beforeText = newKaiDexMessageBuffer.substring(0, splitPoint);
            const afterText = newKaiDexMessageBuffer.substring(splitPoint);
            addItem({
                type: pendingHistoryItemRef.current?.type,
                text: beforeText,
            }, userMessageTimestamp);
            setPendingHistoryItem({ type: "gemini_content", text: afterText });
            newKaiDexMessageBuffer = afterText;
        }
        return newKaiDexMessageBuffer;
    ...
    const handleErrorEvent = useCallback((eventValue, userMessageTimestamp) => {
        ...
        addItem({
            ...
            text: parseAndFormatApiError(eventValue.error, config.getContentGeneratorConfig()?.authType, undefined, config.getModel(), DEFAULT_KAIDEX_FLASH_MODEL),
        }, userMessageTimestamp);
    ...
    const processGeminiStreamEvents = useCallback(async (stream, userMessageTimestamp, signal) => {
        ...
        for await (const event of stream) {
            switch (event.type) {
                case ServerKaiDexEventType.Thought:
                    setThought(event.value);
                    break;
                case ServerKaiDexEventType.Content:
                    geminiMessageBuffer = handleContentEvent(event.value, geminiMessageBuffer, userMessageTimestamp);
                    break;
                case ServerKaiDexEventType.ToolCallRequest:
                    toolCallRequests.push(event.value);
                    break;
                case ServerKaiDexEventType.UserCancelled:
                    handleUserCancelledEvent(userMessageTimestamp);
                    break;
                case ServerKaiDexEventType.Error:
                    handleErrorEvent(event.value, userMessageTimestamp);
                    break;
                case ServerKaiDexEventType.ChatCompressed:
                    handleChatCompressionEvent(event.value);
                    break;
                case ServerKaiDexEventType.ToolCallConfirmation:
                case ServerKaiDexEventType.ToolCallResponse:
                    // do nothing
                    break;
                case ServerKaiDexEventType.MaxSessionTurns:
                    handleMaxSessionTurnsEvent();
                    break;
                case ServerKaiDexEventType.Finished:
                    handleFinishedEvent(event, userMessageTimestamp);
                    break;
                case ServerKaiDexEventType.Citation:
                    handleCitationEvent(event.value, userMessageTimestamp);
                    break;
                case ServerKaiDexEventType.LoopDetected:
                    // handle later because we want to move pending history to history
                    // before we add loop detected message to history
                    loopDetectedRef.current = true;
                    break;
                case ServerKaiDexEventType.Retry:
    ...
    const submitQuery = useCallback(async (query, options, prompt_id) => {
        ...
        try {
            ...
            if (error instanceof UnauthorizedError) {
                ...
                addItem({
                    type: MessageType.ERROR,
                    text: parseAndFormatApiError(getErrorMessage(error) || "Unknown error", config.getContentGeneratorConfig()?.authType, undefined, config.getModel(), DEFAULT_KAIDEX_FLASH_MODEL),
                }, userMessageTimestamp);
            }
        }
        finally {
...
//# sourceMappingURL=useKaiDexStream.js.map