...
import { DEFAULT_KAIDEX_FLASH_LITE_MODEL, getResponseText, } from "@google/kaidex-cli-core";
...
export function usePromptCompletion({ buffer, config, enabled, }) {
    ...
    const generatePromptSuggestions = useCallback(async () => {
        ...
        try {
            ...
            const response = await geminiClient.generateContent(contents, generationConfig, signal, DEFAULT_KAIDEX_FLASH_LITE_MODEL);
            if (signal.aborted) {
                return;
            }
            if (response) {
