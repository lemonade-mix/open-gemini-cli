...
import { DEFAULT_KAIDEX_FLASH_MODEL, IdeClient, ideContext, getErrorMessage, getAllGeminiMdFilenames, AuthType, clearCachedCredentialFile, } from "@google/kaidex-cli-core";
...
import { useKaiDexStream } from "./hooks/useKaiDexStream.js";
...
export const AppContainer = (props) => {
    ...
    const [kaidexMdFileCount, setKaidexMdFileCount] = useState(initializationResult.kaidexMdFileCount);
    ...
    const getEffectiveModel = useCallback(() => {
        if (config.isInFallbackMode()) {
            return DEFAULT_KAIDEX_FLASH_MODEL;
        }
    ...
    const handleAuthSelect = useCallback(async (authType, scope) => {
        if (authType) {
            ...
            if (authType === AuthType.LOGIN_WITH_GOOGLE &&
                ...
                console.log(`
----------------------------------------------------------------
Logging in with Google... Please restart KaiDex CLI to continue.
----------------------------------------------------------------
          `);
                process.exit(0);
            }
    ...
    const { handleSlashCommand, slashCommands, pendingHistoryItems: pendingSlashCommandHistoryItems, commandContext, shellConfirmationRequest, confirmationRequest, } = useSlashCommandProcessor(config, settings, historyManager.addItem, historyManager.clearItems, historyManager.loadHistory, refreshStatic, toggleVimEnabled, setIsProcessing, setKaidexMdFileCount, slashCommandActions);
    const performMemoryRefresh = useCallback(async () => {
        historyManager.addItem({
            type: MessageType.INFO,
            text: "Refreshing hierarchical memory (KAIDEX.md or other context files)...",
        }, Date.now());
        try {
            ...
            setKaidexMdFileCount(fileCount);
            historyManager.addItem({
                type: MessageType.INFO,
    ...
    const { streamingState, submitQuery, initError, pendingHistoryItems: pendingGeminiHistoryItems, thought, cancelOngoingRequest, } = useKaiDexStream(config.getKaiDexClient(), historyManager.history, historyManager.addItem, config, settings, setDebugMessage, handleSlashCommand, shellModeActive, () => settings.merged.general?.preferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, refreshStatic, () => cancelHandlerRef.current());
