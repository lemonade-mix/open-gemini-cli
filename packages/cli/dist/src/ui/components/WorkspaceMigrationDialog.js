...
export function WorkspaceMigrationDialog(props) {
    ...
    if (migrationComplete) {
        return (_jsx(Box, { flexDirection: "column", borderStyle: "round", borderColor: Colors.Gray, padding: 1, children: failedExtensions.length > 0 ? (_jsxs(_Fragment, { children: [_jsxs(Text, { children: ["The following extensions failed to migrate. Please try installing them manually. To see other changes, KaiDex CLI must be restarted. Press ", "'q'", " to quit."] }), _jsx(Box, { flexDirection: "column", marginTop: 1, marginLeft: 2, children: failedExtensions.map((failed) => (_jsxs(Text, { children: ["- ", failed] }, failed))) })] })) : (_jsxs(Text, { children: ["Migration complete. To see changes, KaiDex CLI must be restarted. Press ", "'q'", " to quit."] })) }));
    }
