...
import { Config, loadServerHierarchicalMemory, setGeminiMdFilename as setServerKaiDexMdFilename, getCurrentGeminiMdFilename as getCurrentKaiDexMdFilename, ApprovalMode, DEFAULT_KAIDEX_MODEL, DEFAULT_KAIDEX_EMBEDDING_MODEL, DEFAULT_MEMORY_FILE_FILTERING_OPTIONS, FileDiscoveryService, ShellTool, EditTool, WriteFileTool, } from "@google/kaidex-cli-core";
...
export async function parseArguments(settings) {
    const yargsInstance = yargs(hideBin(process.argv))
        ...
        .command("$0 [promptWords...]", "Launch KaiDex CLI", (yargsInstance) => yargsInstance
        .option("model", {
        alias: "m",
        type: "string",
        description: `Model`,
...
export async function loadCliConfig(settings, extensions, sessionId, argv, cwd = process.cwd()) {
    ...
    if (settings.context?.fileName) {
        setServerKaiDexMdFilename(settings.context.fileName);
    }
    else {
        // Reset to default if not provided in settings.
        setServerKaiDexMdFilename(getCurrentKaiDexMdFilename());
    }
