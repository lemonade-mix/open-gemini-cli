 ...
 */
import { GEMINI_DIR, Storage } from "@google/kaidex-cli-core";
...
export const INSTALL_METADATA_FILENAME = ".kaidex-extension-install.json";
export class ExtensionStorage {
    ...
    static async createTmpDir() {
        return await fs.promises.mkdtemp(path.join(os.tmpdir(), "gemini-extension"));
    }
}
export function getWorkspaceExtensions(workspaceDir) {
    return loadExtensionsFromDir(workspaceDir);
}
async function copyExtension(source, destination) {
...
export function loadExtensionsFromDir(dir) {
...
export async function installExtension(installMetadata, cwd = process.cwd()) {
...
async function loadExtensionConfig(extensionDir) {
...
export async function uninstallExtension(extensionName, cwd = process.cwd()) {
    const installedExtensions = loadUserExtensions();
    if (!installedExtensions.some((installed) => installed.config.name === extensionName)) {
        throw new Error(`Extension "${extensionName}" not found.`);
    }
    removeFromDisabledExtensions(extensionName, [SettingScope.User, SettingScope.Workspace], cwd);
...
export async function updateExtensionByName(extensionName, cwd = process.cwd()) {
    const installedExtensions = loadUserExtensions();
    const extension = installedExtensions.find((installed) => installed.config.name === extensionName);
    if (!extension) {
        throw new Error(`Extension "${extensionName}" not found. Run gemini extensions list to see available extensions.`);
    }
    return await updateExtension(extension, cwd);
}
export async function updateExtension(extension, cwd = process.cwd()) {
    if (!extension.installMetadata) {
    ...
    try {
        ...
        const updatedExtension = loadExtension(updatedExtensionStorage.getExtensionDir());
        if (!updatedExtension) {
 ...
 */
function removeFromDisabledExtensions(name, scopes, cwd = process.cwd()) {
    const settings = loadSettings(cwd);
...
export async function updateAllUpdatableExtensions(cwd = process.cwd()) {
    const extensions = loadExtensions(cwd).filter((extension) => !!extension.installMetadata);
