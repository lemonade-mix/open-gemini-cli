...
// Try to find .env file (works for both bundle and dist)
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const possibleEnvPaths = [
    join(__dirname, "../.env"), // For bundle: bundle/kaidex.js -> .env
    join(__dirname, "../../../.env"), // For dist: dist/src/kaidex.js -> .env
    join(process.cwd(), "PRIVATE/.env"), // If .env was moved to PRIVATE
    join(process.cwd(), ".env"), // Fallback: current working directory
];
let envLoaded = false;
...
if (!envLoaded && process.env["LLM_PROVIDER"]) {
...
import { sessionId, logUserPrompt, AuthType, getOauthClient, uiTelemetryService, } from "@google/kaidex-cli-core";
...
export async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd(), initializationResult) {
    ...
    const instance = render(_jsx(React.StrictMode, { children: _jsx(AppWrapper, {}) }), {
        exitOnCtrlC: false,
...
export async function main() {
    ...
    if (argv.sessionSummary) {
        registerCleanup(() => {
            ...
            writeFileSync(argv.sessionSummary, JSON.stringify({ sessionMetrics: metrics }, null, 2));
        });
    }
...
//# sourceMappingURL=kaidex.js.map