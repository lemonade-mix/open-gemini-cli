...
describe("loadExtensions", () => {
  ...
  it("should load context file path when KAIDEX.md is present", () => {
    createExtension({
      extensionsDir: userExtensionsDir,
      name: "ext1",
      version: "1.0.0",
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(userExtensionsDir, "ext1", "KAIDEX.md"),
    ]);
...
function createExtension({
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, "KAIDEX.md"), "context");
  }
