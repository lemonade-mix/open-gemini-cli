...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    if (!this.currentIde || !this.currentIdeDisplayName) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run KaiDex CLI in one of these supported IDEs: VS Code or VS Code forks`,
        false,
      );
      return;
    }
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      ...
      console.log(
        `[IDE Integration] Running from ${realCwd}, VS Code workspace: ${ideWorkspacePaths.join(", ")}`,
      );
    }
    return { isValid: true };
  }
    ...
    try {
      ...
      try {
        ...
        if (ppidConfig.workspacePath) {
          ...
          const isMatch = workspacePaths.some((workspacePath: string) => {
            const realWorkspacePath = getRealPath(workspacePath);
            return (
              currentCwd === realWorkspacePath ||
          ...
          if (isMatch) {
            logger.debug(
              `Found matching VS Code window via PPID file: ${ppidPortFile}`,
            );
            return ppidConfig;
          } else {
            logger.debug(
              `PPID file workspace mismatch. CWD: ${currentCwd}, Workspace: ${ppidConfig.workspacePath}`,
            );
          }
        }
      } catch (_) {
      ...
      logger.debug(
        `Scanning ${tmpDir} for VS Code windows matching CWD: ${currentCwd}`,
      );
      ...
      for (const file of portFiles) {
        try {
          ...
          if (config.workspacePath) {
            ...
            if (isMatch) {
              logger.debug(
                `Found matching VS Code window: ${file} (workspace: ${config.workspacePath})`,
              );
              return config;
            }
          }
      ...
      logger.debug(
        `No matching VS Code window found for CWD: ${currentCwd}. Checked ${portFiles.length} port files.`,
      );
    } catch (_) {
