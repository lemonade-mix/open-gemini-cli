...
async function authWithLimkc(): Promise<OauthWebLogin> {
  ...
  const loginCompletePromise = new Promise<void>((resolve, reject) => {
    const server = http.createServer(async (req, res) => {
      try {
        if (req.url!.indexOf("/callback") === -1) {
          ...
          reject(
            new FatalAuthenticationError(
              "Callback not received. Unexpected request: " + req.url,
            ),
          );
        }
        ...
        if (qs.get("error")) {
          ...
          reject(
            new FatalAuthenticationError(
              `Login error: ${errorCode}. ${errorDescription}`,
            ),
          );
          ...
          reject(
            new FatalAuthenticationError(
              "State mismatch. Possible CSRF attack or browser session issue.",
            ),
          );
          ...
          try {
            ...
            reject(
              new FatalAuthenticationError(
                `Failed to save token: ${getErrorMessage(error)}`,
              ),
            );
          }
        } else {
          reject(
            new FatalAuthenticationError(
              "No token received. Please try authenticating again.",
            ),
          );
        }
      } catch (e) {
        if (e instanceof FatalAuthenticationError) {
          ...
          reject(
            new FatalAuthenticationError(
              `Unexpected error during authentication: ${getErrorMessage(e)}`,
            ),
          );
        }
      } finally {
    ...
    server.on("error", (err) => {
      reject(
        new FatalAuthenticationError(
          `Callback server error: ${getErrorMessage(err)}`,
        ),
      );
    });
  });
