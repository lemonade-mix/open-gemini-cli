 ...
 */
export function createKaidexUserContent(message: any): Content {
  ...
  console.log(
    hasFunctionResponse ? "functionResponse (tool results)" : "normal message",
    "parts:",
    parts.length,
  );
  ...
  if (hasFunctionResponse) {
    ...
    console.log(
      parts.length,
      "functionResponse parts without Google SDK conversion",
    );
    console.log(
      JSON.stringify(parts, null, 2),
    );
    return { role: "user", parts: parts as Part[] };
  }
...
export class KaiDexChat {
  ...
  async sendMessage(
    ...
    if (!isFunctionResponse(userContent)) {
      ...
      this.chatRecordingService.recordMessage({
        type: "user",
        content: partListUnionToKaidexArray(userMessage as any),
      });
    }
    ...
    try {
      ...
      const apiCall = () => {
        ...
        return this.config.getContentGenerator().generateContent(
          {
            model: modelToUse,
            contents: googleContentArrayToKaidex(requestContents as any),
  ...
  private async *processStreamResponse(
    ...
    console.log(
      lastChunkIsInvalid && !hasReceivedValidChunk,
    );
