...
describe("MCPOAuthTokenStorage", () => {
    ...
    describe("getAllCredentials", () => {
        ...
        it("should load tokens from file successfully", async () => {
            ...
            expect(fs.readFile).toHaveBeenCalledWith(path.join("/mock/home", ".kaidex", "mcp-oauth-tokens.json"), "utf-8");
        });
    ...
    describe("saveToken", () => {
        it("should save token with restricted permissions", async () => {
            ...
            await tokenStorage.saveToken("test-server", mockToken, "client-id", "https://token.url");
            expect(fs.mkdir).toHaveBeenCalledWith(path.join("/mock/home", ".kaidex"), { recursive: true });
            expect(fs.writeFile).toHaveBeenCalledWith(path.join("/mock/home", ".kaidex", "mcp-oauth-tokens.json"), expect.stringContaining("test-server"), { mode: 0o600 });
        });
        ...
        it("should handle write errors gracefully", async () => {
            ...
            await expect(tokenStorage.saveToken("test-server", mockToken)).rejects.toThrow("Disk full");
    ...
    describe("deleteCredentials", () => {
        ...
        it("should remove token file when no tokens remain", async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.join("/mock/home", ".kaidex", "mcp-oauth-tokens.json"));
