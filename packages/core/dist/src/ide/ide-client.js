...
export class IdeClient {
    ...
    async connect() {
        if (!this.currentIde || !this.currentIdeDisplayName) {
            this.setState(IDEConnectionStatus.Disconnected, `IDE integration is not supported in your current environment. To use this feature, run KaiDex CLI in one of these supported IDEs: VS Code or VS Code forks`, false);
            return;
        }
    ...
    static validateWorkspacePath(ideWorkspacePath, currentIdeDisplayName, cwd) {
        ...
        if (!isWithinWorkspace) {
            // Allow IDE integration from any directory - KaiDex is a global agent
            // Just log for debugging, don't block
            console.log(`[IDE Integration] Running from ${realCwd}, VS Code workspace: ${ideWorkspacePaths.join(", ")}`);
        }
        return { isValid: true };
    }
    getPortFromEnv() {
    ...
    async getConnectionConfigFromFile() {
        ...
        const currentCwd = getRealPath(process.cwd());
        try {
            // First try the PPID-based file (legacy approach)
            const ppidPortFile = path.join(tmpDir, `gemini-ide-server-${this.ideProcessInfo.pid}.json`);
            try {
                const ppidContents = await fs.promises.readFile(ppidPortFile, "utf8");
                const ppidConfig = JSON.parse(ppidContents);
                // Check if this window's workspace matches our CWD
                if (ppidConfig.workspacePath) {
                    const workspacePaths = ppidConfig.workspacePath.split(path.delimiter);
                    const isMatch = workspacePaths.some((workspacePath) => {
                        const realWorkspacePath = getRealPath(workspacePath);
                        return (currentCwd === realWorkspacePath ||
                            currentCwd.startsWith(realWorkspacePath + path.sep));
                    });
                    if (isMatch) {
                        logger.debug(`Found matching VS Code window via PPID file: ${ppidPortFile}`);
                        return ppidConfig;
                    }
                    else {
                        logger.debug(`PPID file workspace mismatch. CWD: ${currentCwd}, Workspace: ${ppidConfig.workspacePath}`);
                    }
                }
            }
            catch (_) {
                // PPID file doesn't exist or is invalid, continue to scan all files
            }
            // Scan all port files to find the matching workspace
            logger.debug(`Scanning ${tmpDir} for VS Code windows matching CWD: ${currentCwd}`);
            const files = await fs.promises.readdir(tmpDir);
            const portFiles = files.filter((f) => f.startsWith("gemini-ide-server-"));
            for (const file of portFiles) {
                try {
                    ...
                    if (config.workspacePath) {
                        const workspacePaths = config.workspacePath.split(path.delimiter);
                        const isMatch = workspacePaths.some((workspacePath) => {
                            const realWorkspacePath = getRealPath(workspacePath);
                            return (currentCwd === realWorkspacePath ||
                                currentCwd.startsWith(realWorkspacePath + path.sep));
                        ...
                        if (isMatch) {
                            logger.debug(`Found matching VS Code window: ${file} (workspace: ${config.workspacePath})`);
                            return config;
                        }
                    }
                }
            ...
            logger.debug(`No matching VS Code window found for CWD: ${currentCwd}. Checked ${portFiles.length} port files.`);
        }
        catch (_) {
