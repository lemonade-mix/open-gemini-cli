...
describe("parseAndFormatApiError", () => {
    ...
    it("should format a 429 API error with the default message", () => {
        ...
        const result = parseAndFormatApiError(errorMessage, undefined, undefined, "gemini-2.5-pro", DEFAULT_KAIDEX_FLASH_MODEL);
    ...
    it("should format a 429 API error with the personal message", () => {
        const errorMessage = 'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Rate limit exceeded","status":"RESOURCE_EXHAUSTED"}}';
        const result = parseAndFormatApiError(errorMessage, AuthType.LOGIN_WITH_GOOGLE, undefined, "gemini-2.5-pro", DEFAULT_KAIDEX_FLASH_MODEL);
        expect(result).toContain("[API Error: Rate limit exceeded");
        expect(result).toContain("Possible quota limitations in place or slow response times detected. Switching to the gemini-2.5-flash model");
    });
    ...
    it("should format a 429 API error with Pro quota exceeded message for Google auth (Free tier)", () => {
        ...
        expect(result).toContain("upgrade to a KaiDex Code Assist Standard or Enterprise plan");
    });
    ...
    it("should format a 429 API error with generic quota exceeded message for Google auth", () => {
        ...
        expect(result).not.toContain("You have reached your daily KaiDex 2.5 Pro quota limit");
    });
    ...
    it("should format a 429 API error with Pro quota exceeded message for Google auth (Standard tier)", () => {
        const errorMessage = 'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Gemini 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
        const result = parseAndFormatApiError(errorMessage, AuthType.LOGIN_WITH_GOOGLE, UserTierId.STANDARD, "gemini-2.5-pro", DEFAULT_KAIDEX_FLASH_MODEL);
        expect(result).toContain("[API Error: Quota exceeded for quota metric 'Gemini 2.5 Pro Requests'");
        expect(result).toContain("You have reached your daily gemini-2.5-pro quota limit");
        expect(result).toContain("We appreciate you for choosing KaiDex Code Assist and the KaiDex CLI");
        expect(result).not.toContain("upgrade to a KaiDex Code Assist Standard or Enterprise plan");
    });
    it("should format a 429 API error with Pro quota exceeded message for Google auth (Legacy tier)", () => {
        const errorMessage = 'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Gemini 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
        const result = parseAndFormatApiError(errorMessage, AuthType.LOGIN_WITH_GOOGLE, UserTierId.LEGACY, "gemini-2.5-pro", DEFAULT_KAIDEX_FLASH_MODEL);
        expect(result).toContain("[API Error: Quota exceeded for quota metric 'Gemini 2.5 Pro Requests'");
        expect(result).toContain("You have reached your daily gemini-2.5-pro quota limit");
        expect(result).toContain("We appreciate you for choosing KaiDex Code Assist and the KaiDex CLI");
        expect(result).not.toContain("upgrade to a KaiDex Code Assist Standard or Enterprise plan");
    });
    it("should handle different KaiDex 2.5 version strings in Pro quota exceeded errors", () => {
        ...
        const result25 = parseAndFormatApiError(errorMessage25, AuthType.LOGIN_WITH_GOOGLE, undefined, "gemini-2.5-pro", DEFAULT_KAIDEX_FLASH_MODEL);
        const resultPreview = parseAndFormatApiError(errorMessagePreview, AuthType.LOGIN_WITH_GOOGLE, undefined, "gemini-2.5-preview-pro", DEFAULT_KAIDEX_FLASH_MODEL);
        ...
        expect(result25).toContain("upgrade to a KaiDex Code Assist Standard or Enterprise plan");
        expect(resultPreview).toContain("upgrade to a KaiDex Code Assist Standard or Enterprise plan");
    });
