...
export async function summarizeToolOutput(textToSummarize, geminiClient, abortSignal, maxOutputTokens = 2000) {
    ...
    try {
        const parsedResponse = (await geminiClient.generateContent(googleContentArrayToKaidex(contents), toolOutputSummarizerConfig, abortSignal, DEFAULT_KAIDEX_FLASH_LITE_MODEL));
