...
import { kaidexContentArrayToGoogle, googleContentArrayToKaidex, } from "./typeAdapters.js";
...
async function findLastEditTimestamp(filePath, client) {
    const history = kaidexContentArrayToGoogle((await client.getHistory()) ?? []);
...
`.trim();
    ...
    try {
        const result = await geminiClient.generateJson(googleContentArrayToKaidex(contents), OLD_STRING_CORRECTION_SCHEMA, abortSignal, EditModel, EditConfig, "editCorrector-oldString");
        if (result &&
...
Return ONLY the corrected string in the specified JSON format with the key 'corrected_new_string'. If no adjustment is deemed necessary or possible, return the original_new_string.
  `.trim();
    const contents = [{ role: "user", parts: [{ text: prompt }] }];
    try {
        const result = await geminiClient.generateJson(googleContentArrayToKaidex(contents), NEW_STRING_CORRECTION_SCHEMA, abortSignal, EditModel, EditConfig, "editCorrector-newString");
        if (result &&
...
Return ONLY the corrected string in the specified JSON format with the key 'corrected_new_string_escaping'. If no escaping correction is needed, return the original potentially_problematic_new_string.
  `.trim();
    const contents = [{ role: "user", parts: [{ text: prompt }] }];
    try {
        const result = await geminiClient.generateJson(googleContentArrayToKaidex(contents), CORRECT_NEW_STRING_ESCAPING_SCHEMA, abortSignal, EditModel, EditConfig, "editCorrector-newStringEscaping");
        if (result &&
...
Return ONLY the corrected string in the specified JSON format with the key 'corrected_string_escaping'. If no escaping correction is needed, return the original potentially_problematic_string.
  `.trim();
    const contents = [{ role: "user", parts: [{ text: prompt }] }];
    try {
        const result = await client.generateJson(contents, CORRECT_STRING_ESCAPING_SCHEMA, abortSignal, EditModel, EditConfig, "editCorrector-stringEscaping");
        if (result &&
