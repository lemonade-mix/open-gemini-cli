...
describe("loadServerHierarchicalMemory", () => {
    ...
    describe("when untrusted", () => {
        it("does not load context files from untrusted workspaces", async () => {
            ...
            const { fileCount } = await loadServerHierarchicalMemory(cwd, [], false, new FileDiscoveryService(projectRoot), [], false);
            expect(fileCount).toEqual(0);
        });
        it("loads context from outside the untrusted workspace", async () => {
            await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), "Project root memory"); // Untrusted
            await createTestFile(path.join(cwd, DEFAULT_CONTEXT_FILENAME), "Src directory memory"); // Untrusted
    ...
    it("should load extension context file paths", async () => {
        const extensionFilePath = await createTestFile(path.join(testRootDir, "extensions/ext1/KAIDEX.md"), "Extension memory content");
    ...
    it("should handle multiple directories and files in parallel correctly", async () => {
        // Create multiple test directories with KAIDEX.md files
        const numDirs = 5;
        const createdFiles = [];
