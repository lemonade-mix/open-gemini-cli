...
vi.mock("../core/client.js", () => ({
    KaiDexClient: vi.fn().mockImplementation(function (_config) {
...
describe("editCorrector", () => {
    ...
    describe("ensureCorrectEdit", () => {
        ...
        beforeEach(() => {
            ...
            mockConfigInstance = {
                ...
                getKaiDexMdFileCount: vi.fn(() => configParams.kaidexMdFileCount),
                setKaiDexMdFileCount: vi.fn((count) => {
                    configParams.kaidexMdFileCount = count;
                }),
            ...
            mockKaiDexClientInstance = new KaiDexClient(mockConfigInstance);
            mockKaiDexClientInstance.getHistory = vi.fn().mockResolvedValue([]);
        ...
        describe("Scenario Group 1: originalParams.old_string matches currentContent directly", () => {
            it("Test 1.1: old_string (no literal \\), new_string (escaped by Gemini) -> new_string unescaped", async () => {
                ...
                const result = await ensureCorrectEdit("/test/file.txt", currentContent, originalParams, mockKaiDexClientInstance, abortSignal);
        ...
        describe("Scenario Group 6: Concurrent Edits", () => {
            it("Test 6.1: should return early if file was modified by another process", async () => {
                ...
                mockKaiDexClientInstance.getHistory.mockResolvedValue(history);
                const result = await ensureCorrectEdit(filePath, currentContent, originalParams, mockKaiDexClientInstance, abortSignal);
                expect(result.occurrences).toBe(0);
                expect(result.params).toEqual(originalParams);
            });
        });
    ...
    describe("ensureCorrectFileContent", () => {
        ...
        it("should return content unchanged if no escaping issues detected", async () => {
            ...
            const result = await ensureCorrectFileContent(content, mockKaiDexClientInstance, abortSignal);
