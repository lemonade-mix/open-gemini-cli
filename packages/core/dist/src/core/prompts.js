...
- **Explain Critical Commands:** When executing commands with '${ShellTool.Name}' that modify the file system, codebase, or system state, provide a brief explanation of the command's purpose and potential impact in the SAME response where you call the tool. Prioritize user understanding and safety. You should not ask permission to use the tool; the user will be presented with a confirmation dialogue upon use (you do not need to tell them this).
...
`.trim();
    ...
    const memorySuffix = userMemory && userMemory.trim().length > 0
        ? `\n\n---\n\n${userMemory.trim()}`
        : "";
    const finalPrompt = `${basePrompt}${memorySuffix}`;
    return finalPrompt;
}
/**
