...
describe("Turn", () => {
    ...
    describe("run", () => {
        ...
        it("should yield Error event and report if sendMessageStream throws", async () => {
            ...
            expect(reportError).toHaveBeenCalledWith(error, "Error when talking to KaiDex API", [...historyContent, reqParts], "Turn.run-sendMessageStream");
        });
        ...
        it("should not yield citation event if there is no finish reason", async () => {
            ...
            expect(events.some((e) => e.type === KaiDexEventType.Citation)).toBe(false);
        });
