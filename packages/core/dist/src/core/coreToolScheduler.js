 ...
 */
import { googleArrayToKaidexArray, partListUnionToKaidexArray, } from "../utils/typeAdapters.js";
import { ToolConfirmationOutcome, ApprovalMode, logToolCall, ReadFileTool, ToolErrorType, ToolCallEvent, } from "../index.js";
...
export function convertToFunctionResponse(toolName, callId, llmContent) {
    ...
    if (contentToProcess.functionResponse) {
        if (contentToProcess.functionResponse.response?.["content"]) {
            const stringifiedOutput = getResponseTextFromParts(googleArrayToKaidexArray(contentToProcess.functionResponse.response["content"])) || "";
                ...
                invocation
                    ...
                    if (toolResult.error === undefined) {
                        ...
                        if (typeof content === "string" &&
                            ...
                            ({ content, outputFile } = await truncateAndSaveToFile(content, callId, this.config.storage.getProjectTempDir(), this.config.getTruncateToolOutputThreshold(), this.config.getTruncateToolOutputLines()));
                        }
                        const response = convertToFunctionResponse(toolName, callId, partListUnionToKaidexArray(content));
