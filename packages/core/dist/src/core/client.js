...
import { DEFAULT_KAIDEX_FLASH_MODEL, DEFAULT_THINKING_MODE, } from "../config/models.js";
...
export class KaiDexClient {
    ...
    async startChat(extraHistory) {
        ...
        try {
            ...
            console.log("✨ SYSTEM_INJECT_READY: prompt length:", systemInstruction.length);
            ...
            const generateContentConfigWithThinking = isThinkingSupported(model)
                ...
                : this.generateContentConfig;
            const kaidexChat = new KaiDexChat(this.config, {
                systemInstruction,
            ...
            await reportError(error, "Error initializing KaiDex chat session.", history, "startChat");
    ...
    async *sendMessageStream(request, signal, prompt_id, turns = MAX_TURNS, originalModel) {
        ...
        const lastMessageAfterTurn = historyAfterTurn.length > 0
            ? historyAfterTurn[historyAfterTurn.length - 1]
            : undefined;
        const hasPendingToolCallAfterTurn = !!lastMessageAfterTurn &&
    ...
    async generateJson(contents, schema, abortSignal, model, config = {}, caller = "unknown") {
        ...
        try {
            ...
            console.log(text ? `"${text.slice(0, 100)}..."` : "NULL/EMPTY");
            if (!text) {
                const error = new Error(`API returned an empty response for generateJson (caller: ${caller}).`);
                await reportError(error, `Error in generateJson (caller: ${caller}): API returned an empty response.`, contents, `generateJson-empty-response-${caller}`);
                throw error;
            }
            const prefix = "```json";
            const suffix = "```";
            ...
            try {
                ...
                await reportError(parseError, `Failed to parse JSON response from generateJson (caller: ${caller}).`, {
                    caller,
            ...
            await reportError(error, `Error generating JSON content via API (caller: ${caller}).`, contents, `generateJson-api-${caller}`);
