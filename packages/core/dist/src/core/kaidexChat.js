...
import { partListUnionToKaidexArray, googleContentArrayToKaidex, } from "../utils/typeAdapters.js";
...
/**
 * Creates user content while preserving functionResponse parts for tool results.
 *
 * @remarks
 * Google SDK's createUserContent doesn't handle functionResponse parts correctly
 * for OpenAI-compatible local LLMs. This function detects tool results and preserves
 * them as-is, while using Google SDK's converter for normal messages.
 *
 * @param message - The message or parts to convert
 * @returns Content object with role 'user' and properly formatted parts
 */
export function createKaidexUserContent(message) {
    const parts = Array.isArray(message) ? message : [message];
    // Check if any part contains functionResponse (tool results)
    const hasFunctionResponse = parts.some((part) => part && typeof part === "object" && "functionResponse" in part);
    console.log(hasFunctionResponse ? "functionResponse (tool results)" : "normal message", "parts:", parts.length);
    if (hasFunctionResponse) {
        // Preserve functionResponse parts as-is for OpenAI-compatible format
        console.log(parts.length, "functionResponse parts without Google SDK conversion");
        console.log(JSON.stringify(parts, null, 2));
        return { role: "user", parts: parts };
    }
    // Use Google SDK's conversion for normal messages
    console.log();
...
export class KaiDexChat {
    ...
    async sendMessage(params, prompt_id) {
        ...
        const userContent = createKaidexUserContent(params.message);
        ...
        if (!isFunctionResponse(userContent)) {
            ...
            this.chatRecordingService.recordMessage({
                type: "user",
                content: partListUnionToKaidexArray(userMessage),
            });
        }
        const requestContents = this.getHistory(true).concat(userContent);
        let response;
        try {
            let currentAttemptModel;
            const apiCall = () => {
                ...
                if (this.config.getQuotaErrorOccurred() &&
                    modelToUse === DEFAULT_KAIDEX_FLASH_MODEL) {
                ...
                return this.config.getContentGenerator().generateContent({
                    model: modelToUse,
                    contents: googleContentArrayToKaidex(requestContents),
                    config: { ...this.generationConfig, ...params.config },
                }, prompt_id);
            };
            const onPersistent429Callback = async (authType, error) => {
                if (!currentAttemptModel)
                    return null;
                return await handleFallback(this.config, currentAttemptModel, authType, error);
            };
    ...
    async *processStreamResponse(streamResponse, userInput) {
        ...
        for (let i = 0; i < Math.min(3, receivedChunks.length); i++) {
        }
        ...
        console.log(lastChunkIsInvalid && !hasReceivedValidChunk);
        if (!hasToolCall &&
     ...
     * This is called by external components when tool calls complete, before sending responses to KaiDex.
     */
...
//# sourceMappingURL=kaidexChat.js.map