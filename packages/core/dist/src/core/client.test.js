 ...
 */
// @ts-nocheck - Temporarily bypassing type errors for build
import { describe, it, expect, vi, beforeEach, afterEach, } from "vitest";
import { findIndexAfterFraction, isThinkingDefault, isThinkingSupported, KaiDexClient, } from "./client.js";
import { AuthType, } from "./contentGenerator.js";
import {} from "./kaidexChat.js";
import { CompressionStatus, KaiDexEventType, Turn, } from "./turn.js";
...
describe("Gemini Client (client.ts)", () => {
    ...
    beforeEach(async () => {
        ...
        // Because the KaiDexClient constructor kicks off an async process (startChat)
        ...
        client = new KaiDexClient(mockConfig);
        await client.initialize();
        vi.mocked(mockConfig.getKaiDexClient).mockReturnValue(client);
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    describe("generateJson", () => {
        it("should call generateContent with the correct parameters", async () => {
            ...
            await client.generateJson(contents, schema, abortSignal, DEFAULT_KAIDEX_FLASH_MODEL);
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith({
                model: DEFAULT_KAIDEX_FLASH_MODEL,
                config: {
                    abortSignal,
...
\`\`\`
      `.trim();
        ...
        it("should yield MaxSessionTurns and stop when session turn limit is reached", async () => {
            ...
            expect(events).toEqual([{ type: KaiDexEventType.MaxSessionTurns }]);
        ...
        it("should not call checkNextSpeaker when turn.run() yields a value then an error", async () => {
            ...
            const mockCheckNextSpeaker = vi.mocked(checkNextSpeaker);
            const mockStream = (async function* () {
                yield { type: KaiDexEventType.Content, value: "some content" };
                yield {
                    type: KaiDexEventType.Error,
                    value: { error: { message: "test error" } },
                };
    ...
    describe("generateContent", () => {
        it("should call generateContent with the correct parameters", async () => {
            ...
            const abortSignal = new AbortController().signal;
            await client.generateContent(contents, generationConfig, abortSignal, DEFAULT_KAIDEX_FLASH_MODEL);
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith({
                model: DEFAULT_KAIDEX_FLASH_MODEL,
                config: {
                    abortSignal,
        ...
        it("should use current model from config for content generation", async () => {
            ...
            await client.generateContent(contents, {}, new AbortController().signal, DEFAULT_KAIDEX_FLASH_MODEL);
