...
describe("OAuthCredentialStorage", () => {
    ...
    const oldFilePath = "/mock/home/.kaidex/oauth.json";
    beforeEach(() => {
    ...
    describe("loadCredentials", () => {
        ...
        it("should throw an error if loading fails", async () => {
            vi.spyOn(storage, "getCredentials").mockRejectedValue(new Error("Loading error"));
            await expect(oauthStorage.loadCredentials()).rejects.toThrow("Failed to load OAuth credentials");
        });
    ...
    describe("saveCredentials", () => {
        ...
        it("should throw an error if access_token is missing", async () => {
            ...
            await expect(oauthStorage.saveCredentials(invalidCredentials)).rejects.toThrow("Attempted to save credentials without an access token.");
        });
    });
    describe("clearCredentials", () => {
        ...
        it("should throw an error if clearing from HybridTokenStorage fails", async () => {
            vi.spyOn(storage, "deleteCredentials").mockRejectedValue(new Error("Deletion error"));
            await expect(oauthStorage.clearCredentials()).rejects.toThrow("Failed to clear OAuth credentials");
        });
    });
});
