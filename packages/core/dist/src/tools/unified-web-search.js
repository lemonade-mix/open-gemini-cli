...
class UnifiedWebSearchToolInvocation extends BaseToolInvocation {
     ...
     */
    async performTavilySearch(query, apiKey, maxResults, signal) {
     ...
     */
    async performGoogleCustomSearch(query, apiKey, searchEngineId, maxResults, signal) {
     ...
     */
    async performGoogleGeminiSearch(query, signal) {
        const geminiClient = this.config.getKaiDexClient();
        const response = await geminiClient.generateContent([{ role: "user", parts: [{ text: query }] }], { tools: [{ googleSearch: {} }] }, signal, DEFAULT_KAIDEX_FLASH_MODEL);
        return response;
    }
    async execute(signal) {
        ...
        if (tavilyApiKey) {
            try {
                const searchResult = await this.performTavilySearch(this.params.query, tavilyApiKey, maxResults, signal);
        ...
        if (googleApiKey && googleSearchEngineId) {
            try {
                const searchResult = await this.performGoogleCustomSearch(this.params.query, googleApiKey, googleSearchEngineId, maxResults, signal);
            ...
            catch (googleCustomError) {
                // Google Custom Search failed, log and try Gemini fallback
                console.warn(`Google Custom Search failed: ${getErrorMessage(googleCustomError)}`);
            }
        }
        // STRATEGY 3: Tertiary fallback to Google Gemini Search (Gemini provider only)
        try {
            const response = await this.performGoogleGeminiSearch(this.params.query, signal);
            ...
            if (googleApiKey && googleSearchEngineId)
                attemptedMethods.push("Google Custom Search");
            const fallbackNote = attemptedMethods.length > 0
            ...
            if (googleApiKey && googleSearchEngineId) {
                ...
                errorDetails.push("Google Custom Search: GOOGLE_SEARCH_ENGINE_ID not set");
            }
            else if (!googleApiKey) {
                errorDetails.push("Google Custom Search: GOOGLE_API_KEY not set");
            }
            attemptedMethods.push("Google Gemini Search");
            errorDetails.push(`Google Gemini Search: ${getErrorMessage(googleGeminiError)}`);
...
export class UnifiedWebSearchTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(UnifiedWebSearchTool.Name, "WebSearch", "Performs a web search and returns results. Uses three-tier automatic fallback: (1) Tavily AI (if TAVILY_API_KEY set - best for LLMs), (2) Google Custom Search (if GOOGLE_API_KEY and GOOGLE_SEARCH_ENGINE_ID set - works with all LLM providers), (3) Google Gemini Search (Gemini provider only). For best results with local LLMs, set TAVILY_API_KEY (free 1,000 searches/month at https://tavily.com).", Kind.Search, {
            type: "object",
            properties: {
                query: {
                    type: "string",
                ...
                maxResults: {
                    type: "number",
                    description: "Maximum number of search results to return (1-20). Default: 5. Only applies when using Tavily search.",
                    minimum: 1,
                    maximum: 20,
                    default: 5,
                },
...
//# sourceMappingURL=unified-web-search.js.map