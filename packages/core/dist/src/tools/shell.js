...
class ShellToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput, terminalColumns, terminalRows) {
        ...
        try {
            ...
            let outputChunks = [cumulativeOutput];
            let lastUpdateTime = Date.now();
            let isBinaryStream = false;
            const { result: resultPromise } = await ShellExecutionService.execute(commandToExecute, cwd, (event) => {
                if (!updateOutput) {
                    return;
                }
            ...
            if (summarizeConfig && summarizeConfig[ShellTool.Name]) {
                const summary = await summarizeToolOutput(llmContent, this.config.getKaiDexClient(), signal, summarizeConfig[ShellTool.Name].tokenBudget);
                return {
                    llmContent: summary,
