...
class WebSearchToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal) {
        ...
        try {
            const response = await geminiClient.generateContent([{ role: "user", parts: [{ text: this.params.query }] }], { tools: [{ googleSearch: {} }] }, signal, DEFAULT_KAIDEX_FLASH_MODEL);
...
export class WebSearchTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(WebSearchTool.Name, "GoogleSearch", "Performs a web search using Google Search (via the KaiDex API) and returns the results. This tool is useful for finding information on the internet based on a query.", Kind.Search, {
            type: "object",
            properties: {
                query: {
                    type: "string",
