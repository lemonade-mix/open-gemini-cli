...
KaiDex CLI can integrate with your IDE to provide a more seamless and context-aware experience. This integration allows the CLI to understand your workspace better and enables powerful features like native in-editor diffing.
...
- **Native Diffing:** When KaiDex suggests code modifications, you can view the changes directly within your IDE's native diff viewer. This allows you to review, edit, and accept or reject the suggested changes seamlessly.

- **VS Code Commands:** You can access KaiDex CLI features directly from the VS Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Gemini CLI: Run`: Starts a new KaiDex CLI session in the integrated terminal.
...
When you run KaiDex CLI inside a supported editor, it will automatically detect your environment and prompt you to connect. Answering "Yes" will automatically run the necessary setup, which includes installing the companion extension and enabling the connection.

### 2. Manual Installation from CLI

If you previously dismissed the prompt or want to install the extension manually, you can run the following command inside KaiDex CLI:
...
- **For Visual Studio Code:** Install from the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.kaidex-cli-vscode-ide-companion).
...
When enabled, KaiDex CLI will automatically attempt to connect to the IDE companion extension.
...
When you ask KaiDex to modify a file, it can open a diff view directly in your editor.
...
If you are using KaiDex CLI within a sandbox, please be aware of the following:
...
- **Message:** `🔴 Disconnected: Failed to connect to IDE companion extension in [IDE Name]. Please ensure the extension is running. To install the extension, run /ide install.`
  - **Cause:** KaiDex CLI could not find the necessary environment variables (`GEMINI_CLI_IDE_WORKSPACE_PATH` or `GEMINI_CLI_IDE_SERVER_PORT`) to connect to the IDE. This usually means the IDE companion extension is not running or did not initialize correctly.
  - **Solution:**
...
- **Message:** `🔴 Disconnected: Directory mismatch. KaiDex CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:** `IDE integration is not supported in your current environment. To use this feature, run KaiDex CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running KaiDex CLI in a terminal or environment that is not a supported IDE.
  - **Solution:** Run KaiDex CLI from the integrated terminal of a supported IDE, like VS Code.

- **Message:** `No installer is available for IDE. Please install the KaiDex CLI Companion extension manually from the marketplace.`
