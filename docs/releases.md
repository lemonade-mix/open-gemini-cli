...
```bash
npm install -g @google/kaidex-cli@preview
```
...
```bash
npm install -g @google/kaidex-cli@latest
```
...
```bash
npm install -g @google/kaidex-cli@nightly
```
...
- `npx -y @google/kaidex-cli@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @google/kaidex-cli@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @google/kaidex-cli && npm uninstall -g @google/kaidex-cli && npm cache clean --force &&  npm install @google/kaidex-cli@<version>`
