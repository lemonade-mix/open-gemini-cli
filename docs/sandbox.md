# Sandboxing in the KaiDex CLI

This document provides a guide to sandboxing in the KaiDex CLI, including prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the KaiDex CLI:
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect gemini-cli due to automatic exclusion. Use `.kaidex/.env` files for gemini-cli specific debug settings.
