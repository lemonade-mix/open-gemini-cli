...
- **`model.name`** (string):
  - **Description:** The KaiDex model to use for conversations.
  - **Default:** `undefined`
...
- **`context.fileFiltering.respectGeminiIgnore`** (boolean):
  - **Description:** Respect .kaidexignore files when searching.
  - **Default:** `true`
...
Configures logging and metrics collection for KaiDex CLI. For more information, see [Telemetry](../telemetry.md).
...
**Environment Variable Exclusion:** Some environment variables (like `DEBUG` and `DEBUG_MODE`) are automatically excluded from being loaded from project `.env` files to prevent interference with gemini-cli behavior. Variables from `.kaidex/.env` files are never excluded. You can customize this behavior using the `advanced.excludedEnvVars` setting in your `settings.json` file.
...
- **Hierarchical Loading and Precedence:** The CLI implements a sophisticated hierarchical memory system by loading context files (e.g., `KAIDEX.md`) from several locations. Content from files lower in this list (more specific) typically overrides or supplements content from files higher up (more general). The exact concatenation order and final context can be inspected using the `/memory show` command. The typical loading order is:
  1.  **Global Context File:**
      - Location: `~/.kaidex/<configured-context-filename>` (e.g., `~/.kaidex/KAIDEX.md` in your user home directory).
