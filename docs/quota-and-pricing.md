# KaiDex CLI: Quotas and Pricing

KaiDex CLI offers a generous free tier that covers the use cases for many individual developers. For enterprise / professional usage, or if you need higher limits, there are multiple possible avenues depending on what type of account you use to authenticate.
...
This article outlines the specific quotas and pricing applicable to the KaiDex CLI when using different authentication methods.
...
For users who authenticate by using their Google account to access KaiDex Code Assist for individuals. This includes:

- 1000 model requests / user / day
- 60 model requests / user / minute
- Model requests will be made across the KaiDex model family as determined by KaiDex CLI.
...
### Log in with KaiDex API Key (Unpaid)

If you are using a KaiDex API key, you can also benefit from a free tier. This includes:
...
If you use up your initial number of requests, you can upgrade your plan to continue to benefit from KaiDex CLI by using the [Standard or Enterprise editions of KaiDex Code Assist](https://cloud.google.com/products/gemini/pricing) by signing up [here](https://goo.gle/set-up-gemini-code-assist). Quotas and pricing are based on a fixed price subscription with assigned license seats. For predictable costs, you can log in with Google. This includes:
...
If you hit your daily request limits or exhaust your KaiDex Pro quota even after upgrading, the most flexible solution is to switch to a pay-as-you-go model, where you pay for the specific amount of processing you use. This is the recommended path for uninterrupted access.

To do this, log in using a KaiDex API key or Vertex AI.
...
## Google One and Ultra plans, KaiDex for Workspace plans

These plans currently apply only to the use of KaiDex web-based products provided by Google-based experiences (for example, the KaiDex web app or the Flow video editor). These plans do not apply to the API usage which powers the KaiDex CLI. Supporting these plans is under active consideration for future support.
...
## KaiDex API vs. Vertex

- KaiDex API (gemini developer api): This is the fastest way to use the KaiDex models directly.
- Vertex AI: This is the enterprise-grade platform for building, deploying, and managing KaiDex models with specific security and control requirements.
