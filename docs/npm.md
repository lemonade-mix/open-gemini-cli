...
This monorepo contains two main packages: `@google/kaidex-cli` and `@google/kaidex-cli-core`.

## `@google/kaidex-cli`

This is the main package for the KaiDex CLI. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@google/kaidex-cli-core`. This means that whether a user installs the package with `npm install -g @google/kaidex-cli` or runs it directly with `npx @google/kaidex-cli`, they are using this single, self-contained executable.

## `@google/kaidex-cli-core`

This package contains the core logic for interacting with the KaiDex API. It is responsible for making API requests, handling authentication, and managing the local cache.
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @google/kaidex-cli`.
